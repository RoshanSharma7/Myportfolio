...
export default function Home() {
  ...
  // Fetch profile, projects, and experiences together — loading stays true
  // until all three have settled, so the skeleton doesn't disappear early
  // while some sections are still empty.
  useEffect(function() {
    async function fetchAll() {
      try {
        ...
        const [profileResult, projectsResult, experiencesResult] = results

        if (profileResult.status === 'fulfilled' && profileResult.value.data && profileResult.value.data.length > 0) {
          setProfile(profileResult.value.data[0])
        ...
        if (experiencesResult.status === 'fulfilled') {
          ...
          console.error('Failed to fetch experiences:', experiencesResult.reason)
        }
      } finally {
        setLoading(false)
      }
  ...
  if (loading) return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Hero skeleton */}
        <SkeletonBox width="160px" height="0.9rem" style={{ marginBottom: '0.8rem' }} />
        <SkeletonBox width={isMobile ? '260px' : '480px'} height={isMobile ? '2.6rem' : '4rem'} style={{ marginBottom: '1rem' }} />
        <SkeletonBox width={isMobile ? '220px' : '380px'} height={isMobile ? '1.6rem' : '2.2rem'} style={{ marginBottom: '1.5rem' }} />
        <SkeletonBox width="100%" height="0.9rem" style={{ marginBottom: '0.5rem', maxWidth: '600px' }} />
        <SkeletonBox width="80%" height="0.9rem" style={{ marginBottom: '2rem', maxWidth: '480px' }} />

        <div style={{ display: 'flex', gap: '1rem', marginBottom: '3rem' }}>
          <SkeletonBox width="170px" height="3rem" radius="8px" />
          <SkeletonBox width="130px" height="3rem" radius="8px" />
        </div>

        <div style={{ display: 'flex', gap: isMobile ? '2rem' : '3rem', marginBottom: '3rem' }}>
          {[1, 2, 3].map(function(i) {
            return (
              <div key={i}>
                <SkeletonBox width="50px" height="2.2rem" style={{ marginBottom: '0.5rem' }} />
                <SkeletonBox width="90px" height="0.8rem" />
              </div>
            )
          })}
        </div>

        {/* Latest Project + Work Experience card skeletons */}
        <SkeletonBox width="140px" height="1rem" style={{ marginBottom: '1rem' }} />
        <div
          className="card"
          style={{
            padding: '1.2rem',
        ...
        >
          <SkeletonBox width="80px" height="1.5rem" radius="999px" />
          <SkeletonBox width="60%" height="1.2rem" />
          <SkeletonBox width="100%" height="0.85rem" />
          <SkeletonBox width="85%" height="0.85rem" />
        </div>

        <SkeletonBox width="160px" height="1rem" style={{ marginBottom: '1rem' }} />
        <div
          className="card"
          style={{
            padding: '1.2rem',
        ...
        >
          <SkeletonBox width="50%" height="1.2rem" />
          <SkeletonBox width="35%" height="0.9rem" />
          <SkeletonBox width="130px" height="1.5rem" radius="999px" />
          <SkeletonBox width="100%" height="0.85rem" />
        </div>
      </div>
    </div>
